# N��wa Backup �� Project Engineering Memory
...
**Status:** ? COMPLETE (user confirmed "T2-08�͵�������")
...
## 19. P-03 — Directory & Boundary Semantics
...
## 22. P-06 — CLI/App/Tauri UI Integration
...
- E2E end-to-end validation: backup → close → reopen → restore → SHA-256 verify
...
## 23. P-07 — Delete Old Flat File Modules
...
| File | Status |
|------|--------|
| `src/backup.rs` | ✅ Deleted |
| `src/restore.rs` | ✅ Deleted |
| `src/storage.rs` | ✅ Deleted |
| `src/manifest.rs` | ✅ Deleted |
| `src/verify.rs` | ✅ Deleted |
| `src/list.rs` | ✅ Deleted |
| `src/prune.rs` | ✅ Deleted |
| `src/repository/legacy/adapter.rs` | ✅ Deleted |
| `src/repository/legacy/mod.rs` | ✅ Deleted |
| `tests/backup_restore_tests.rs` | ✅ Deleted |
| `tests/gui_integration_tests.rs` | ✅ Deleted |
| ~45 temp Python scripts | ✅ Cleaned |
...
## 24. P-08 — Full Regression + Residual Scan
...
| Layer | Scope | Result |
|-------|-------|--------|
| Layer 1 — Source | `src/**/*.rs`: flat_file/FlatFile/old module refs | ✅ CLEAN |
| Layer 2 — CLI | `src/cli.rs`: Verify/List/Prune commands | ✅ REMOVED |
| Layer 3 — UI | `ui/src/**/*.ts*`: flat-file references | ✅ CLEAN |
| Layer 4 — Tests | `tests/**/*.rs`: flat-file references | ✅ CLEAN |
| Layer 5 — Config | `Cargo.toml`: old [[test]] sections | ✅ CLEAN |
| Layer 6 — Docs | AGENTS.md/README.md: historical references | ✅ RETAINED (historical record) |
...
|------|--------|
| `cargo fmt --check` | ✅ PASS |
| `cargo clippy --features repository --all-targets -- -D warnings` | ✅ PASS |
| `cargo test --features repository` | ✅ **370 passed, 0 failed** |
| `cargo build --features repository` | ✅ PASS |

### Bugs Fixed During P-08

1. `src/main.rs:144`: JSON format string — regular string `"{{` prematurely terminated by `"`. Fixed with `r#"..."#` raw string.
2. `src/history.rs:183-186`: Orphaned doc comments from deleted `rebuild_from_manifest` — triggered clippy `empty_line_after_doc_comments`.
3. `src/repository/file_restore_reader.rs:57`: SQL had duplicate columns (`status, instance_path` repeated) + `row.get()` index mismatch with `RestorePointRecord` field order — caused 13 test failures.
...
Application Service Layer (src/app/)
    |
Repository Engine (src/repository/) — ONLY storage backend
```
...
|---------|-------------|--------|
| `nuwa init [--config <path>]` | Init config file | ✅ |
| `nuwa backup --source <path> --repo <path> [--compress] [--json]` | Repository backup | ✅ |
| `nuwa restore --backup <repo_path> --dest <path> [--overwrite] [--json]` | Repository restore | ✅ |
| `nuwa history --dest <repo_path> [--limit N] [--json]` | Operation history | ✅ |
| `nuwa schedule (create|list|delete)` | Task Scheduler | ✅ |
...
| 7 | Manifest error (obsolete — repository-only) |
